                                        ...
                                        .pipe(
                                            ...
                                            flatMap(_ =>
                                                of(_throwError)
                                                    .pipe(
                                                        filter(__ => !!__),
                                                        flatMap(__ => throwError(new Error('Missing server state last id')))
                                                    )
                                            )
                                        ),
                                    hasKey
                                        ...
                                        .pipe(
                                            ...
                                            flatMap(_ =>
                                                of(_throwError)
                                                    .pipe(
                                                        filter(__ => !!__),
                                                        flatMap(__ => throwError(new Error('Missing server state data')))
                                                    )
                                            )
                                        ),
                                    hasKey
            ...
            .pipe(
                flatMap((obs: Observable<string[]>) =>
                    merge(
                        ...
                        obs.pipe(
                            filter(_ => !_ || !_.length),
                            flatMap(_ => throwError(new Error(`Missing header '${headerName}' value inside request to generate state key`)))
                        )
                    )
                )
            );
